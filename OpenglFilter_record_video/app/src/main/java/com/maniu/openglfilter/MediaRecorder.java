...
public class MediaRecorder {
    ...
    public void start(float speed) {
        ...
        //线程
        //創建OpenGL 的 環境
        HandlerThread handlerThread = new HandlerThread("thread-codec-gl");
        handlerThread.start();
     ...
     * 编码数据
     *
     * @param endOfStream ： 是否停止
     */
    private void startCodec(boolean endOfStream) {
        Log.d(TAG, "startCodec: endOfStream : " + endOfStream);
    ...
    public void stop() {
        ...
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                Log.i(TAG, "handler post stop: -----start");
                mMediaCodec.stop();
                startCodec(true);
                Log.i(TAG, "handler post stop: -----end");
                mMediaCodec.reset();
                mMediaCodec.release();
                mMediaCodec = null;
                mMuxer.stop();
